...
release = hslider("[3]release", 0.1, 0, 1, 0.1);
cutoff = hslider("[4]cutoff[style:knob]", 5000, 50, 10000, 1) : si.smoo;
resonance = hslider("[5]resonance[style:knob]", 1, 1, 2, .9) : si.smoo;
env = en.ar(attack, release, gate);
pitch = ba.midikey2hz(note);
osc = os.pulsetrain(pitch, 0.5);
...
cutoff = hslider("[4]cutoff[style:knob]", 5000, 50, 10000, 1) : si.smoo;
resonance = hslider("[5]resonance[style:knob]", 1, 1, 2, .9) : si.smoo;
lfoFreq = hslider("[6]LFO[style:knob]", 1, 0, 80, 0.1) : si.smoo;
cutoffAmount = hslider("[7]LFO->cutoff[style:knob]", 0, 0, 1, 0.1) : si.smoo;
pitchAmount = hslider("[8]LFO->pitch[style:knob]", 0, 0, 1, 0.1) : si.smoo;
env = en.ar(attack, release, gate);
