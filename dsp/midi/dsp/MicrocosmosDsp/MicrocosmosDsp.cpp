...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("attack", &fHslider2, FAUSTFLOAT(0.100000001f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.100000001f));
		ui_interface->addHorizontalSlider("duty", &fHslider1, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.100000001f));
		ui_interface->addButton("gate", &fButton0);
		ui_interface->addHorizontalSlider("note", &fHslider0, FAUSTFLOAT(69.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(127.0f), FAUSTFLOAT(1.0f));
		ui_interface->addHorizontalSlider("release", &fHslider3, FAUSTFLOAT(0.100000001f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.100000001f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow0 = std::max<float>((440.0f * std::pow(2.0f, (0.0833333358f * (float(fHslider0) + -69.0f)))), 23.4489498f);
		float fSlow1 = std::max<float>(20.0f, std::fabs(fSlow0));
		float fSlow2 = (fConst1 / fSlow1);
		float fSlow3 = (fConst2 * fSlow1);
		float fSlow4 = std::max<float>(0.0f, std::min<float>(2047.0f, (fConst0 * (float(fHslider1) / fSlow0))));
		float fSlow5 = std::floor(fSlow4);
		float fSlow6 = (fSlow5 + (1.0f - fSlow4));
		int iSlow7 = int(fSlow4);
		float fSlow8 = (fSlow4 - fSlow5);
		int iSlow9 = (iSlow7 + 1);
		float fSlow10 = std::max<float>(1.0f, (fConst0 * float(fHslider2)));
		float fSlow11 = (1.0f / fSlow10);
		float fSlow12 = float(fButton0);
		float fSlow13 = (1.0f / std::max<float>(1.0f, (fConst0 * float(fHslider3))));
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			iVec0[0] = 1;
			fRec0[0] = (fSlow3 + (fRec0[1] - std::floor((fSlow3 + fRec0[1]))));
			float fTemp0 = mydsp_faustpower2_f(((2.0f * fRec0[0]) + -1.0f));
			fVec1[0] = fTemp0;
			float fTemp1 = (fSlow2 * (float(iVec0[1]) * (fTemp0 - fVec1[1])));
			fVec2[(IOTA & 4095)] = fTemp1;
			fVec3[0] = fSlow12;
			iRec1[0] = (((iRec1[1] + (iRec1[1] > 0)) * (fSlow12 <= fVec3[1])) + (fSlow12 > fVec3[1]));
			float fTemp2 = float(iRec1[0]);
			float fTemp3 = ((fTemp1 - ((fSlow6 * fVec2[((IOTA - iSlow7) & 4095)]) + (fSlow8 * fVec2[((IOTA - iSlow9) & 4095)]))) * std::max<float>(0.0f, std::min<float>((fSlow11 * fTemp2), ((fSlow13 * (fSlow10 - fTemp2)) + 1.0f))));
	...
	FAUST_ADDHORIZONTALSLIDER("attack", fHslider2, 0.10000000000000001f, 0.0f, 1.0f, 0.10000000000000001f);
	FAUST_ADDHORIZONTALSLIDER("duty", fHslider1, 0.5f, 0.0f, 1.0f, 0.10000000000000001f);
	FAUST_ADDBUTTON("gate", fButton0);
	FAUST_ADDHORIZONTALSLIDER("note", fHslider0, 69.0f, 0.0f, 127.0f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("release", fHslider3, 0.10000000000000001f, 0.0f, 1.0f, 0.10000000000000001f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, attack, "attack", fHslider2, 0.10000000000000001f, 0.0f, 1.0f, 0.10000000000000001f) \
		p(HORIZONTALSLIDER, duty, "duty", fHslider1, 0.5f, 0.0f, 1.0f, 0.10000000000000001f) \
		p(BUTTON, gate, "gate", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, note, "note", fHslider0, 69.0f, 0.0f, 127.0f, 1.0f) \
		p(HORIZONTALSLIDER, release, "release", fHslider3, 0.10000000000000001f, 0.0f, 1.0f, 0.10000000000000001f) \
